...
//********************************
// Übungsblatt 2		
// Aufgabe 1(a&b): als CountConnectedComponents 
//********************************

		public int CountConnectedComponents(){
			List<int> ConnectedComponent = DFS_Stack (0);
			Console.WriteLine ("");
			Console.WriteLine ("Die Zusammenhangskomponente (Start bei 0):");
			for (int t = 0; t < ConnectedComponent.Count; t++) Console.Write ("-> " + ConnectedComponent [t] + " ");
			int size = ConnectedComponent.Count; 
			return size;
		}

// Ende CountConnectedComponents
//********************************	
// MaxConnectedComponents 
//********************************

		public void MaxConnectedComponents(){
			if (V < 0) throw new ArgumentException("Number of vertices should not be negative\n");
			Stack<int> allNodes = new Stack<int>();
			for (int i = 0; i < V; i++) if (adj[i].Count != 0) allNodes.Push (i); 	
			List<int> ConnectedComponent = new List<int>();
			List<int> maxConnectedComponent = new List <int> ();
			while (allNodes.Count > 0) {
				int nextNode = allNodes.Pop ();
				ConnectedComponent = DFS_Stack (nextNode);
				if (ConnectedComponent.Count > maxConnectedComponent.Count) maxConnectedComponent = ConnectedComponent;	
			}
			Console.WriteLine ("Die maximale Zusammenhangskomponente vom Graphen ist:");
			for (int t = 0; t < maxConnectedComponent.Count; t++) Console.Write ("-> " + maxConnectedComponent [t] + " ");
		}

// Ende MaxConnectedComponents
